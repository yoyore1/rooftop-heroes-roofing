...
function allPhotos(lead) {
  const seen = new Set();
  return [lead.photo_url, ...(lead.photo_urls || [])].filter(u => {
...
async function sendEmail(lead) {
  ...
  const text = [
    ...
    dash ? `Dashboard: ${dash}` : "",
    ...photos.map((u, i) => `Photo ${photos.length > 1 ? i + 1 : ""}:    ${u}`.trim()),
  ].filter(Boolean).join("\n");

  const r = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
    body: JSON.stringify({ from, to, subject, text, html: emailHtml(lead, rows, dash, photos) }),
  });
  if (!r.ok) throw new Error(`resend ${r.status}: ${(await r.text()).slice(0, 200)}`);
}

function emailHtml(lead, rows, dash, photos = []) {
  ...
  // Build photo grid with HTML tables — CSS Grid is not supported in most email clients
  let photoGridHtml = "";
  if (photos.length) {
    const pairs = [];
    ...
    photoGridHtml = `
        <tr><td style="padding:14px 24px 0">
          <div style="color:#6b7280;font-size:13px;margin-bottom:8px">Roof Photo${photos.length > 1 ? "s" : ""} (${photos.length})</div>
          <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
            ${pairs.map(pair => `<tr>${pair.map((u, pi) =>
              `<td style="width:${pair.length === 1 ? "100" : "50"}%;padding:0 ${pi === 0 && pair.length > 1 ? "4" : "0"}px 8px 0;vertical-align:top">
                <img src="${esc(u)}" alt="Roof photo" width="${pair.length === 1 ? 512 : 256}" style="width:100%;max-width:${pair.length === 1 ? "512" : "256"}px;height:auto;border-radius:8px;display:block">
              </td>`).join("")}</tr>`).join("")}
          </table>
        </td></tr>`;
  }
...
async function sendSms(lead) {
  ...
  const photos = allPhotos(lead);
  console.log(`[notify] sms photo_url=${lead.photo_url ? "yes" : "no"} photo_urls=${JSON.stringify(lead.photo_urls)} total=${photos.length}`);
  ...
  const body = [
    ...
    preview,
    photos.length > 1 ? `+ ${photos.length} photos attached` : "",
  ].filter(Boolean).join("\n");

  const params = new URLSearchParams({ To: to, From: from, Body: body });
  // Twilio MMS supports up to 10 media URLs
  photos.slice(0, 10).forEach((url, i) => params.append(`MediaUrl${i + 1}`, url));
