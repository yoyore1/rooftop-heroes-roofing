// Lead notifications: a richly structured email (Resend), a concise SMS
// (Twilio), and a Web Push alert to the owner's installed dashboard. Channels
// are independent — if one is unconfigured or errors, the others still fire,
// and the lead is already saved in the database either way.

import { listSubscriptions, deleteSubscriptionByEndpoint } from "./db.js";
import { pushConfigured, sendOne } from "./push.js";

const TZ = "America/Chicago"; // Arkansas — Central Time

export async function notifyLead(lead) {
  const results = await Promise.allSettled([sendEmail(lead), sendSms(lead), sendPush(lead)]);
  const labels = ["email", "sms", "push"];
  results.forEach((r, i) => {
...
async function sendEmail(lead) {
  ...
  const toRaw = process.env.LEAD_EMAIL;
  if (!key || !toRaw) return; // not configured — skip silently
  // LEAD_EMAIL may be a comma-separated list — Resend's `to` accepts an array.
...
async function sendSms(lead) {
  ...
  // LEAD_SMS_TO may be a comma-separated list — text every recipient.
  ...
  // Send to each number independently — one failing shouldn't block the rest.
  const results = await Promise.allSettled(recipients.map((to) => {
    ...
    return fetch(`https://api.twilio.com/2010-04-01/Accounts/${sid}/Messages.json`, {
      ...
      if (!r.ok) throw new Error(`twilio ${r.status}: ${txt.slice(0, 200)}`);
      try { const j = JSON.parse(txt); console.log(`[notify] sms ${to}: sid=${j.sid} status=${j.status} error=${j.error_code}`); } catch {}
    });
  }));

  const failed = results.filter((r) => r.status === "rejected");
  if (failed.length === recipients.length) throw failed[0].reason; // all failed — surface it
  if (failed.length) console.error(`[notify] sms partial failure ${failed.length}/${recipients.length}:`, failed[0].reason?.message);
}

/* ---------- Web Push (browser notifications) ---------- */

async function sendPush(lead) {
  if (!pushConfigured()) return; // not configured — skip silently

  let subs = [];
  try {
    subs = await listSubscriptions();
  } catch (e) {
    console.error("[notify] push: could not list subscriptions:", e.message);
    return;
  }
  if (!subs.length) return;

  const parts = [lead.service || "Roofing", lead.phone].filter(Boolean);
  ...
  const results = await Promise.allSettled(subs.map(async (row) => {
    ...
    const res = await sendOne(sub, payload);
    // Prune subscriptions the push service has retired.
    if (res?.gone) { try { await deleteSubscriptionByEndpoint(row.endpoint); } catch {} }
  }));

  const failed = results.filter((r) => r.status === "rejected");
  if (failed.length) console.error(`[notify] push ${failed.length}/${subs.length} failed:`, failed[0].reason?.message);
}
