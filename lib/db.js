...
// Save (or refresh) a browser push subscription. Upserts on the unique
// `endpoint` so re-enabling alerts on the same device never duplicates.
export async function insertSubscription(sub) {
  ...
  if (!r.ok) throw new Error(`supabase sub insert ${r.status}: ${(await r.text()).slice(0, 200)}`);
  const rows = await r.json();
  return rows[0];
}

export async function listSubscriptions() {
  const r = await fetch(`${URL}/rest/v1/push_subscriptions?select=*`, { headers: headers() });
  if (!r.ok) throw new Error(`supabase sub list ${r.status}: ${(await r.text()).slice(0, 200)}`);
  return r.json();
}
...
export async function deleteSubscriptionByEndpoint(endpoint) {
  ...
  if (!r.ok) throw new Error(`supabase sub delete ${r.status}: ${(await r.text()).slice(0, 200)}`);
}
